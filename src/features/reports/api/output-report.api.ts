...
export type OutputReportParams = {
  fromDate: string;
  toDate: string;
  companyId?: string;
};
...
export async function getKpiDashboard(params: OutputReportParams) {
  return apiRequest<KpiDashboard>(`${apiEndpoints.reports.kpiDashboard}?${toQuery(params)}`, {
    method: "GET",
    timeoutMs: 60_000,
  });
}

export async function getOutputReportSummary(params: OutputReportParams) {
  return apiRequest<OutputReportSummary>(`${apiEndpoints.reports.outputSummary}?${toQuery(params)}`, {
    method: "GET",
    timeoutMs: 60_000,
  });
}

export async function downloadOutputReport(params: OutputReportParams) {
  return apiDownloadBlob(`${apiEndpoints.reports.outputExport}?${toQuery(params)}`, {
    method: "GET",
    timeoutMs: 120_000,
  });
}

function toQuery(params: OutputReportParams) {
